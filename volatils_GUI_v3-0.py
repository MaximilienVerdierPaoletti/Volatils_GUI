...
class App(customtkinter.CTkToplevel):
    ...
    def Plot_Data(self):
        ...
        list_std = stand_meas.columns.to_list()
        stand = stand_meas[[m for m in list_std if 'Ratio' in m or 'Err Mean' in m]]
        ...
        for i in range(0, len(stand)):
            ...
            list_std = difflib.get_close_matches(Xname, ls_stand, n=1, cutoff=0)  # Retrieve good name of standards (even if variations are present)
            d = pd.concat([d, stand_true.loc[stand_true.NAME == list_std[0]]], ignore_index=True)
    ...
    def linear_regression(self, stand_meas, data):
        ...
        for i in range(0, len(rat)):
            ...
            ax1[i].text(ax1[i].get_xlim()[1] * 0.05, ax1[i].get_ylim()[1] * 0.85, r'$True = Meas \times$ ' + str(int(np.round(mod.params.iloc[1]))) + ' + ' + str(int(np.round(mod.params.iloc[0]))))
            ax1[i].set_xlabel(rat[i])
            ax1[i].set_box_aspect(1)

            if i != 1: ax1[i].set_ylabel('True concentration (ppm)')
            else: ax1[i].set_ylabel('True concentration (wt%)')

            # ============ Saving text strings containing statistical tests summaries
            text_stat = text_stat + '\n\n' + '----------------------------------------' + T[i] + '-----------------------------------' + '\n\n' + str(mod.summary())

            # ============ Saving parameters of linear regression
            summary[i, 0] = mod.params.iloc[1]  # slope
            summary[i, 1] = mod.params.iloc[0]  # origin
            summary[i, 2] = T_test
        ...
        volatils = np.zeros((len(meas), len(rat)*2))
        Er_comp = np.zeros((len(meas), len(rat)*5))
        Var = pd.DataFrame()
        sig = []
        Er_lc = []
        lc = []
        ...
        data_corr = pd.concat([meas, Er_comp], axis=1)
